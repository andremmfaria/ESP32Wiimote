...
- static assets are compiled into dedicated translation units in `src/wifi/web/`
- `GET /openapi.json` is not filesystem-backed
- the router builds the OpenAPI JSON from per-route metadata embedded in `kRoutes[]`
- dynamic documented routes that are not dispatched through `kRoutes[]` are described by a supplemental metadata table in the router

Static asset packaging model:

- each browser asset is stored in its own committed C++ source file under `src/wifi/web/`
- `src/wifi/web/index_html.cpp` provides the `/` and `/index.html` payload
- `src/wifi/web/app_js.cpp` provides the `/app.js` payload
- `src/wifi/web/styles_css.cpp` provides the `/styles.css` payload
- `src/wifi/web/web_assets.h` exposes the shared declarations consumed by the router
- this keeps Arduino CLI builds single-step because no filesystem image generation or upload step is required

Request flow:

1. Static route short-circuit (`/`, `/index.html`, `/app.js`, `/styles.css`, `/openapi.json`)
...
- startup order is Wi-Fi layer -> static page routes -> API routes
- `/openapi.json` is generated by the router and does not depend on any packaged asset file
