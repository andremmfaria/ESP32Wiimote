  ...
  switch (controllerReportState) {
    ...
    case REPORT_STATE_WAIT_ACK_OUT_REPORT:
      ...
      if ((data[1] == 0x22) && (data[4] == 0x16)) {
        if (data[5] == 0x00) {
          wiimote_write_memory(ch, CONTROL_REGISTER, 0xA400FB, (const uint8_t[]){0x00}, 1);
    ...
    case REPORT_STATE_WAIT_READ_COTRLLER_TYPE:
      ...
      if ((data[1] == 0x22) && (data[4] == 0x16)) {
        if (data[5] == 0x00) {
          wiimote_read_memory(ch, CONTROL_REGISTER, 0xA400FA, 6);
      ...
      if (data[1] == 0x21) {
        if (memcmp(data + 5, (const uint8_t[]){0x00, 0xFA}, 2) == 0) {
          if (memcmp(data + 7, (const uint8_t[]){0x00, 0x00, 0xA4, 0x20, 0x00, 0x00}, 6) == 0) {
            ...
            wiimote_state_set_nunchuk_connected(true);
            if (wiimote_state_get_use_accelerometer()) {
              wiimote_set_reporting_mode(ch, 0x35, false);
            } else {
              wiimote_set_reporting_mode(ch, 0x32, false);
            }
          }
          controllerReportState = REPORT_STATE_INIT;
        }
